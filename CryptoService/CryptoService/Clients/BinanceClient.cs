...
public sealed class BinanceClient : IBinanceClient
{
    ...
    public async Task<decimal> GetBitcoinPriceAsync(string symbol, CancellationToken cancellationToken)
    {
        ...
        request.Headers.Add("User-Agent", "CryptoService/1.0");
        using var response = await _httpClient.SendAsync(request, cancellationToken);
        response.EnsureSuccessStatusCode();
        var result = await response.Content.ReadFromJsonAsync<BinancePriceDto>(cancellationToken: cancellationToken)
                     ?? throw new Exception("Failed to deserialize Binance response");

        if (!decimal.TryParse(result.Price, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out var price))
